...
import Eval.LispError (LispError (BadSpecialForm, CaseNotMatched, NotFunction, NumArgs, TypeMismatch))
import Eval.LispVal (LispVal (..))
...
eval env (List ((Atom specialName) : rest)) = evalSpecialForm env specialName rest
eval _ val = return val

evalSpecialForm :: Env -> String -> [LispVal] -> IOThrowsError LispVal
evalSpecialForm _ "quote" [val] = return val
evalSpecialForm env "if" [pred', conseq, alt] = ifSpecialForm env pred' conseq alt
evalSpecialForm env "case" rest = caseSpecialForm env rest
evalSpecialForm env "cond" rest = condSpecialForm env rest
evalSpecialForm env "define" [Atom var, form] =
  eval env form >>= defineVar env var
evalSpecialForm env fnName args = mapM (eval env) args >>= liftThrows . apply fnName
